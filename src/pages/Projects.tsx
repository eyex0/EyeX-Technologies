...
export function ProjectsPage() {
  return <ModulePage title="Projects" subtitle="Tasks · Teams · Timeline" tabs={[
    ...
    { key: "tasks", label: "Tasks", render: () => (
      <TableCard title="Tasks" columns={[
        { key: "task", label: "Task" },{ key: "assignee", label: "Assignee" },{ key: "priority", label: "Priority" },{ key: "due", label: "Due", align: "right" },
      ]} rows={[
    ...
    { key: "time", label: "Timeline", render: () => (
      <Card title="Timeline">
        <div className="p-5 space-y-4">
          {m.projects.map((p) => (
            <div key={p.name}>
              <div className="flex justify-between text-xs mb-2"><span className="text-white">{p.name}</span><span className="font-mono text-muted-foreground">{p.due}</span></div>
              <div className="h-1 bg-border rounded"><div className="h-full bg-white" style={{width:`${p.progress}%`}}/></div>
            </div>
          ))}
        </div>
      </Card>
    )},
    { key: "teams", label: "Teams", render: () => (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {["Engineering","Design","Ops"].map((t) => (
          <div key={t} className="bento-card rounded-lg p-5">
            ...
            <div className="text-[10px] font-mono text-muted-foreground uppercase mt-1">12 members · 4 projects</div>
          </div>
        ))}
      </div>
    )},
        ...
        {["Backlog","In Progress","Review","Done"].map((col) => (
          <div key={col} className="bento-card rounded-lg p-4">
            ...
            <div className="space-y-2">
              {[1,2,3].map((i) => (
                <div key={i} className="border border-border rounded p-3 text-xs text-white hover:bg-secondary/40">Task {col.slice(0,3).toUpperCase()}-{i}</div>
              ))}
            </div>
          </div>
        ))}
    ...
    { key: "an", label: "Analytics", render: () => (
      <Card title="Velocity"><BarChart data={Array.from({length:8},(_,i)=>({label:`S${i+1}`,value:20+Math.random()*40}))}/></Card>
    )},
  ]}/>;
}
