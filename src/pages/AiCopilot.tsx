...
export function AiCopilotPage() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      text: "Hi — I'm your Copilot. Ask about revenue, customers, deals, projects, or your entire business.",
    },
  ]);
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            <AnimatePresence initial={false}>
              {messages.map((msg, i) => (
                ...
                >
                  <div
                    className={`max-w-[80%] rounded-lg px-4 py-3 text-sm whitespace-pre-wrap leading-relaxed shadow-sm ${msg.role === "user" ? "bg-white text-black font-medium" : "text-white bg-white/5 border border-white/10"}`}
                  >
                    {msg.text}
                  </div>
                </motion.div>
