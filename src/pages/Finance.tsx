  ...
  return <ModulePage title="Finance" subtitle="Revenue · Expenses · Forecast" tabs={[
    { key: "over", label: "Overview", render: () => (
      <>
        ...
        <Card title="Cash flow — 12mo"><BarChart data={["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"].map((l,i)=>({label:l,value:40+i*4+Math.random()*10}))}/></Card>
      </>
    )},
    ...
    { key: "bud", label: "Budgets", render: () => (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {[["Engineering","$1.2M","78%"],["Sales","$800K","92%"],["Marketing","$400K","64%"],["Ops","$200K","41%"]].map(([n,b,p]) => (
          <div key={n} className="bento-card rounded-lg p-5">
            <div className="flex justify-between mb-3"><span className="text-white text-sm">{n}</span><span className="text-muted-foreground text-xs font-mono">{b}</span></div>
            <div className="h-1 bg-border rounded"><div className="h-full bg-white" style={{width:p as string}}/></div>
            <div className="text-[10px] font-mono text-muted-foreground uppercase mt-2">{p} used</div>
          </div>
        ))}
      </div>
    )},
