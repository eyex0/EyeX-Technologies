...
export function FinancePage() {
  ...
  const [finData, setFinData] = useState<{ revenue: number; expenses: number; months: any[] }>({
    revenue: 0,
    expenses: 0,
    months: [],
  });
            ...
            <>
              <KpiRow
                items={[
                  {
                    label: "Revenue",
                    value: `$${(finData.revenue / 1000).toFixed(1)}K`,
                    delta: "+12.4%",
                    icon: "payments",
                  },
                  {
                    label: "Expenses",
                    value: `$${(finData.expenses / 1000).toFixed(1)}K`,
                    delta: "+4.2%",
                    icon: "trending_down",
                  },
                  {
              ...
              <Card title="Revenue vs Expenses">
                {finData.months.length > 0 ? (
                  <div className="p-4">
                    <BarChart
                      data={finData.months.map((m) => ({ label: m.label, value: m.revenue }))}
                    />
                  </div>
                ) : (
                  <div className="p-8 text-center text-muted-foreground text-sm">
                    {loading
                      ? "Loading..."
                      : "No transaction data yet. Add transactions to see cash flow."}
                  </div>
                )}
              </Card>
            </>
