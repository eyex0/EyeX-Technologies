    ...
    <AppShell title="Analytics" subtitle="Cross-module intelligence">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card title="Traffic — 12w" className="lg:col-span-2">
          <BarChart data={Array.from({length:12},(_,i)=>({label:`W${i+1}`,value:20+Math.floor(Math.random()*80)}))} />
        </Card>
        <Card title="Top Channels">
          <div className="p-5 space-y-3">
            {[["Organic","42%"],["Direct","28%"],["Paid","18%"],["Referral","8%"],["Social","4%"]].map(([n,v]) => (
              <div key={n} className="flex justify-between text-xs"><span className="text-white">{n}</span><span className="font-mono text-muted-foreground">{v}</span></div>
            ))}
          </div>
        </Card>
      </div>
