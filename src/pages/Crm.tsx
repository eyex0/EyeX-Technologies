            ...
            <>
              <KpiRow
                items={[
                  {
                    label: "Customers",
                    value: summary.isLoading ? "—" : String(s?.totalCustomers ?? 0),
                    delta: `${s?.activeCustomers ?? 0} active`,
                    icon: "groups",
                  },
                  {
                    label: "Active",
                    value: summary.isLoading ? "—" : String(s?.activeCustomers ?? 0),
                    icon: "check_circle",
                  },
                  {
                    label: "Pipeline",
                    value: summary.isLoading ? "—" : formatCurrency(s?.pipelineValue ?? 0),
                    icon: "trending_up",
                  },
                  {
                    label: "Conversion",
                    value: summary.isLoading ? "—" : `${(s?.conversionRate ?? 0).toFixed(1)}%`,
                    icon: "emoji_events",
                  },
                ]}
              />
                ...
                <TableCard
                  ...
                  columns={[
                    ...
                    { key: "lifetime_value", label: "LTV", align: "right" },
                    {
                      key: "status",
                      label: "Status",
                      render: (r: (typeof customerRows)[number]) => (
            ...
            customers.isLoading ? (
              ...
              <TableCard
                ...
                columns={[
                  ...
                  { key: "email", label: "Primary" },
                  { key: "lifetime_value", label: "ACV", align: "right" },
                ]}
                rows={customerRows}
              />
            ),
              ...
              <TableCard
                ...
                columns={[
                  ...
                  { key: "source", label: "Source" },
                  {
                    key: "status",
                    label: "Status",
                    render: (r: (typeof leadRows)[number]) => (
                      <Badge tone={statusTone(r.status)}>{r.status}</Badge>
                    ),
                  },
                ]}
            ...
            deals.isLoading ? (
              ...
              <TableCard
                ...
                columns={[
                  ...
                  { key: "close_date", label: "Close", align: "right" },
                ]}
                rows={dealRows}
              />
            ),
            ...
            deals.isLoading ? (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="bento-card rounded-lg p-4 space-y-3">
