...
export function NotificationsPage() {
  ...
  useEffect(() => {
    ...
    const sub = supabase.channel("notifications").on("postgres_changes", {
      ...
      <Card title={`Notifications (${notifications.length})`} icon="notifications">
        <div className="p-2">
          {loading ? (
            ...
            <div className="text-center text-muted-foreground text-sm py-8">No notifications yet</div>
          ) : (
            notifications.map((n: any) => (
              <div key={n.id} className="flex items-start gap-3 px-3 py-3 border-b border-border last:border-0 hover:bg-secondary/40 rounded">
                ...
                <button onClick={() => dismiss(n.id)} className="text-[10px] font-mono uppercase text-muted-foreground hover:text-white">Dismiss</button>
              </div>
            ))
          )}
        </div>
