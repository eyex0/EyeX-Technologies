...
export function HrPage() {
  ...
  useEffect(() => {
    Promise.all([DatabaseService.getEmployees(), DatabaseService.getDepartments()])
      .then(([emp, dept]) => {
        setEmployees(emp);
        setDepartments(dept);
      })
              ...
              <TableCard
                ...
                columns={[
                  ...
                  { key: "position", label: "Position" },
                  {
                    key: "dept_name",
                    label: "Department",
                    render: (r: any) => r.departments?.name || "-",
                  },
              ...
              {(departments.length > 0
                ...
                <div key={d.name || d.id} className="bento-card rounded-lg p-5">
                  ...
                  <div className="text-[10px] font-mono text-muted-foreground">
                    Budget: ${(d.budget / 1000).toFixed(0)}K
                  </div>
                </div>
              ))}
            </div>
          ...
          render: () => (
            <TableCard
              ...
              rows={employees.map((e) => ({
                ...e,
                salary: `$${Number(e.salary).toLocaleString()}/yr`,
              }))}
            />
          ),
        },
            ...
            <Card title="Attendance overview">
              <BarChart
                data={["Mon", "Tue", "Wed", "Thu", "Fri"].map((l) => ({
                  label: l,
                  value: Math.round(activeEmployees * 0.9 + Math.random() * activeEmployees * 0.1),
                }))}
              />
            </Card>
          ),
