...
export function SalesPage() {
  ...
  const statusTone = (status: string) => {
    ...
    if (s === "fulfilled" || s === "completed" || s === "delivered" || s === "paid")
      return "success" as const;
    if (s === "pending" || s === "processing" || s === "draft" || s === "sent")
            ...
            <>
              <KpiRow
                items={[
                  {
                    label: "Revenue MTD",
                    value: summary.isLoading ? "—" : formatCurrency(totalRevenue),
                    delta: `${summary.data?.completedOrders ?? 0} completed`,
                    icon: "payments",
                  },
                  {
                    label: "Orders",
                    value: summary.isLoading ? "—" : String(totalOrders),
                    delta: `${summary.data?.pendingOrders ?? 0} pending`,
                    icon: "shopping_bag",
                  },
                  {
                    label: "AOV",
                    value: summary.isLoading ? "—" : formatCurrency(avgOrderValue),
                    icon: "receipt",
                  },
                  {
                    label: "Invoices",
                    value: financeSummary.isLoading
                      ? "—"
                      : String(financeSummary.data?.totalInvoiced ?? 0),
                    delta: `${financeSummary.data?.overdueInvoices ?? 0} overdue`,
                    icon: "emoji_events",
                  },
                ]}
              />
              <Card title="Revenue by order">
                {orders.isLoading ? (
                  ...
                  <BarChart
                    data={orderRows.slice(0, 12).map((o, i) => ({
                      label: o.order_number,
                      value:
                        parseFloat(o.total.replace(/[$K,M]/g, "")) *
                        (o.total.includes("M") ? 1000000 : o.total.includes("K") ? 1000 : 1),
                    }))}
                  />
                )}
              </Card>
              ...
              <TableCard
                ...
                columns={[
                  ...
                  { key: "total", label: "Amount" },
                  {
                    key: "status",
                    label: "Status",
                    render: (r: (typeof orderRows)[number]) => (
                ...
                {summary.isLoading ? (
                  ...
                  <BarChart
                    data={[
                      ...
                      {
                        label: "Revenue",
                        value: Math.round((summary.data?.totalRevenue ?? 0) / 1000),
                      },
