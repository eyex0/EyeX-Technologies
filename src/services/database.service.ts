...
export const DatabaseService = {
  ...
  async createDataset(name: string, description?: string): Promise<Dataset> {
    ...
    const { data, error } = await supabase
      .from("datasets")
      .insert({ name, description, user_id: session.user.id, organization_id: orgId })
      .select()
      .single();
    if (error) throw error;
    return data;
  ...
  async saveDashboard(title: string, layout: any): Promise<Dashboard> {
    ...
    const { data, error } = await supabase
      .from("dashboards")
      .insert({ title, layout, user_id: session.user.id, organization_id: orgId })
      .select()
      .single();
    if (error) throw error;
    return data;
  ...
  async createInvoice(
    ...
    const { data, error } = await supabase
      .from("invoices")
      .insert({ ...invoice, organization_id: orgId })
      .select()
      .single();
    if (error) throw error;
    return data;
  ...
  async getRevenueExpenses(): Promise<{
    revenue: number;
    expenses: number;
    ...
    const revenue = txns
      .filter((t) => t.type === "credit" || t.category === "revenue")
      .reduce((s, t) => s + Number(t.amount), 0);
    const expenses = txns
      .filter((t) => t.type === "debit" || t.category === "expense")
      .reduce((s, t) => s + Number(t.amount), 0);
  ...
  async getProjectTasks(projectId?: string): Promise<ProjectTask[]> {
    ...
    let query = supabase
      .from("project_tasks")
      .select("*, profiles(full_name), projects!inner(organization_id)")
